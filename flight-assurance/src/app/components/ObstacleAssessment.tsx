...
const ObstacleAssessment: React.FC<ObstacleAssessmentProps> = ({ flightPlan, onDataProcessed, }) => 
    ...
    useEffect(() => {
      const processTerrainData = async () => {
        try {
          ...
          const distances = sampledPoints.map((_, idx) => idx * interval / 1000); // Distances in km
          ...
          for (let idx = 0; idx < coordinates.length; idx++) {
            if (idx === 0) {
              ...
              const segment = turf.lineString([coordinates[idx - 1], coordinates[idx]]); // Segment between waypoints
    ...
    const fetchTerrainElevation = async (lng: number, lat: number): Promise<number> => {
      try {
        ...
        console.error("Failed to fetch or decode terrain elevation:", { lng, lat, error });
