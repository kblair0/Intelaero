...
import ELOSGridAnalysis, { ELOSGridAnalysisRef } from "./ELOSGridAnalysis";
...
const Calculator: React.FC = () => {
  ...
  const [flightPlanDistance, setFlightPlanDistance] = useState<number | null>(
    null
  );
  ...
  const [gcsLocation, setGcsLocation] = useState<{
    lng: number;
    lat: number;
    elevation: number | null;
  } | null>(null);
  ...
  const [repeaterLocation, setRepeaterLocation] = useState<{
    lng: number;
    lat: number;
    elevation: number | null;
  } | null>(null);
  const [rawFlightPlan, setRawFlightPlan] =
    useState<GeoJSON.FeatureCollection | null>(null); // Uploaded but unprocessed
  const [processedFlightPlan, setProcessedFlightPlan] =
    useState<GeoJSON.FeatureCollection | null>(null); // To be used for assessments
  ...
  const calculateReserveFlightTime = (): string => {
    ...
    const reserveTimeInMinutes =
      totalFlightTimeInMinutes - flightPlanTimeInMinutes;
          ...
          <div className="bg-white p-4 rounded-md shadow-md mt-4">
            ...
            {flightPlanDistance !== null && (
              <p className="text-sm mb-2">
                ...
                <span className="font-bold">
                  {flightPlanDistance.toFixed(2)} km
                </span>
              </p>
            )}
            ...
            <p className="text-sm mb-2">
              ...
              <span
                className={`font-bold ${
                  parseFloat(calculateBatteryReserve()) < 0
                    ? "text-red-500"
                    : "text-green-500"
                }`}
              >
            ...
            <div className="bg-white p-4 rounded-md shadow-md mt-4">
              ...
              {averageDraw && (
                <p className="text-sm mb-2">
                  Average Draw:{" "}
                  <span className="font-bold">
                    {(averageDraw * 60).toFixed(2)} mAh/min
                  </span>
                </p>
              )}
                ...
                <div className="overflow-x-auto">
                  <table className="w-full border-collapse border border-gray-500 text-sm text-black">
                    ...
                    <tbody>
                      {phaseData
                        .filter(
                          (phase) => phase.Phase !== "Total Flight Summary"
                        )
                        .map((phase, index) => (
                          <tr
                            key={`phase-${index}`}
                ...
                <button
                  onClick={() => {
                    if (rawFlightPlan) {
                      ...
                      console.warn(
                        "No flight plan is available for obstacle assessment!"
                      );
                    }
                  }}
            ...
            <div className="mt-4 bg-white p-4 rounded-md shadow-md">
              <div className="flex items-center justify-between mb-4">
                ...
                <label className="toggle-switch">
                  <input type="checkbox" onChange={handleToggleElosLayer} />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-lg font-medium">
                        {elosState.stats.visibleCells} /{" "}
                        {elosState.stats.totalCells} cells
                      </p>
                    </div>
                    <div>
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <label className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          onChange={() =>
                            layerManager.toggleLayerVisibility("gcs-grid-layer")
                          }
                    ...
                    <div className="flex items-center space-x-2 mb-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setMarkerElevationOffsets((prev) => ({
                            ...prev,
                          ...
                          if (mapRef.current && gcsLocation) {
                            mapRef.current.runElosAnalysis({
                              ...
                              location: {
                                ...gcsLocation,
                                elevation:
                                  (gcsLocation.elevation || 0) +
                                  markerElevationOffsets.gcs,
                              },
                          ...
                          if (mapRef.current && observerLocation) {
                            mapRef.current.runElosAnalysis({
                              ...
                              location: {
                                ...observerLocation,
                                elevation:
                                  (observerLocation.elevation || 0) +
                                  markerElevationOffsets.observer,
                              },
                          ...
                          if (mapRef.current && repeaterLocation) {
                            mapRef.current.runElosAnalysis({
                              ...
                              location: {
                                ...repeaterLocation,
                                elevation:
                                  (repeaterLocation.elevation || 0) +
                                  markerElevationOffsets.repeater,
                              },
              ...
              {rawFlightPlan && mapRef.current && (
                <ELOSGridAnalysis
                  ...
                  onSuccess={async (result) => {
                    console.log(
                      "Calculator: onSuccess called with result:",
                      result
                    );
                    setElosState((prev) => ({
                      ...prev,
